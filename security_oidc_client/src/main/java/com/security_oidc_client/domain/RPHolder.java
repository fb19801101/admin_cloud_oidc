...
import com.security_oidc_client.domain.shared.BeanProvider;
...
public class RPHolder implements Serializable {
     ...
     * 应用API注册信息
     */
    private static final String API_PROGRAMS = "apiPrograms";
    ...
    public static Map<String, RPHolder> getApis(Map<String, Map<String, Object>> map) {
        Map<String, RPHolder> apis = new HashMap<>();
        for(Map.Entry x : map.entrySet()) {
            apis.put(x.getKey().toString(), new RPHolder((HashMap)x.getValue()));
        }

        return apis;
    }

    public static String apisToJSON(RPHolder rpHolder, Map<String, RPHolder> map) {
        JSONObject json = new JSONObject(new LinkedHashMap<>());
        json.put(CLIENT_NAME, rpHolder.getClientName());
        json.put(CLIENT_FULL_NAME, rpHolder.getClientFullName());
        json.put(CLIENT_ID, rpHolder.getClientId());
        json.put(CLIENT_SECRET, rpHolder.getClientSecret());
        json.put(CLIENT_REDIRECT_URIS, rpHolder.getClientRedirectUris());
        json.put(API_CLIENT_ID, rpHolder.getApiClientId());
        json.put(API_CLIENT_SECRET, rpHolder.getApiClientSecret());
        json.put(API_SCOPE, rpHolder.getApiScope());
        json.put(API_RESOURCE_ID, rpHolder.getApiResourceId());
        json.put(API_NAME, rpHolder.getApiName());
        json.put(API_SCOPE_CALL, rpHolder.getApiScopeCall());
        json.put(WEB_URI_PC, rpHolder.getWebUriPc());
        json.put(WEB_URI_APP, rpHolder.getWebUriApp());
        json.put(WEB_API_URI, rpHolder.getWebApiUri());
        json.put(WEB_API_TODO, rpHolder.getWebApiTodo());
        json.put(WEB_API_INFO, rpHolder.getWebApiInfo());
        json.put(DISCOVERY_ENDPOINT, rpHolder.getDiscoveryEndpoint());
        json.put(CLIENT_PROVIDER_ID, rpHolder.getClientProviderId());
        json.put(CLIENT_USER_ID, rpHolder.getClientUserId());
        json.put(CLIENT_USER_AUTH, rpHolder.getClientUserAuth());
        json.put(CLIENT_USER_TYPE, rpHolder.getClientUserType());
        json.put(VUE_REDIRECT_URIS, rpHolder.getVueRedirectUris());
        JSONObject apis = new JSONObject(new LinkedHashMap<>());
        for(Map.Entry x : map.entrySet()) {
            String k = x.getKey().toString();
            RPHolder v = (RPHolder)x.getValue();
            JSONObject api = new JSONObject(new LinkedHashMap<>());
            api.put(CLIENT_NAME, v.getClientName());
            api.put(CLIENT_FULL_NAME, v.getClientFullName());
            api.put(CLIENT_ID, v.getClientId());
            api.put(CLIENT_SECRET, v.getClientSecret());
            api.put(CLIENT_REDIRECT_URIS, v.getClientRedirectUris());
            api.put(API_CLIENT_ID, v.getApiClientId());
            api.put(API_CLIENT_SECRET, v.getApiClientSecret());
            api.put(API_SCOPE, v.getApiScope());
            api.put(API_RESOURCE_ID, v.getApiResourceId());
            api.put(API_NAME, v.getApiName());
            api.put(API_SCOPE_CALL, v.getApiScopeCall());
            api.put(WEB_URI_PC, v.getWebUriPc());
            api.put(WEB_URI_APP, v.getWebUriApp());
            api.put(WEB_API_URI, v.getWebApiUri());
            api.put(WEB_API_TODO, v.getWebApiTodo());
            api.put(WEB_API_INFO, v.getWebApiInfo());
            api.put(DISCOVERY_ENDPOINT, v.getDiscoveryEndpoint());
            api.put(CLIENT_PROVIDER_ID, v.getClientProviderId());
            api.put(CLIENT_USER_ID, v.getClientUserId());
            api.put(CLIENT_USER_AUTH, v.getClientUserAuth());
            api.put(CLIENT_USER_TYPE, v.getClientUserType());
            api.put(VUE_REDIRECT_URIS, v.getVueRedirectUris());
            apis.put(k, api);
        }
        json.put(API_PROGRAMS, apis);
