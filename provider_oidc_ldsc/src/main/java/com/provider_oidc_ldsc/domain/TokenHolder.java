...
import com.provider_oidc_ldsc.service.dto.CRCCUserInfoDto;
...
//    @PostConstruct
    ...
    public static TokenEntity getToken(RPHolder rpHolder) throws Exception {
        if(rpHolder !=null && rpHolder.isConfigRP()) {
            ...
            String regApiUrl = Application.getRegApiUrl();
            HttpResponse response = HttpUtils.doGet(regApiUrl, "oauth/token", "get", headers, queryMap);
    ...
    public static String doGet(String path) throws Exception {
        ...
        String hrApiUrl = Application.getHrApiUrl();
        HttpResponse response = HttpUtils.doGet(hrApiUrl, path, "get", headers, null);
        if (response.getStatusLine().getStatusCode()==200){
...
//            }
    ...
    public static AccessTokenDto loadAccessToken(RPHolder rpHolder) {
        if(rpHolder !=null && rpHolder.isConfigRP()) {
            ...
            String regApiUrl = Application.getRegApiUrl();
            UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(regApiUrl+"oauth/token").queryParams(param);
...
//                        System.out.println("AsyncRestTemplate Success: getObjListAsyncApi --> size: "+list.size()+" time: " + DateUtils.currentTimeMillis());
        ...
        com.google.common.util.concurrent.ListenableFuture<HttpEntity<String>> ans =
                listeningExecutorService.submit(new Callable<HttpEntity<String>>() {
                    ...
                    public HttpEntity<String> call() throws Exception {
                        RestTemplate restTemplate = new RestTemplate();
                        return restTemplate.getForEntity("http://127.0.0.1:8080/atimeout?name=aaa&age=19", String.class);
                    }
                });
    ...
    public void token() {
        ...
        try {
            ...
            RsaJsonWebKey key = JwtUtils.generalKeyFromUri(regApiUrl+"discovery/certs", "b8fd74c8-acbc-4778-afa9-7564daeea73f");

            if (key != null && token != null) {
                Claims claims = JwtUtils.parserJwt(key.getRsaPublicKey(), token.getAccessToken());
                boolean is = claims != null;
            }
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("RestTemplate Exception: loadAccessToken --> " + e.getMessage());
