...
//                return new CRCCUserInfoDto();
    ...
    public  String clientLogout(HttpServletRequest request) {
        ...
        String openidLogoutUrl = Application.getOpenidLogoutUrl();

        return "redirect:"+openidLogoutUrl;
    }
     ...
     * @throws Exception
     */
    @GetMapping(value = {"auth_callback", "authorization_callback"})
...
//        System.out.println("Auth Callback Task begin.  time: " + DateUtils.formatCurrentLocalDateTime(DateUtils.DATETIME_FORMAT.HOUR_MILLIS));
        ...
        if(userInfoDto != null) {
            ...
            if(providerId != null && providerId.length() > 0 && userId != null) {
                ...
                redirectAttributes.addAttribute("userType", "browse");

                Application application = new Application();
                application.setLoginProviderId(providerId);
                application.setLoginUserId(userId);
                application.setLoginUserType("browse");
                if (VueRetController.authLoginEntity(providerId, userId, authEntityRepository) != null) {
                    VueRetController.asyncAuthLoginOrgDelete(providerId, userId, authEntityRepository, postChangeLogEntityRepository, orgLogEntityRepository, false);
                    ...
                    redirectAttributes.addAttribute("userAuth", "has_auth");
                    if( !PortalData.isSso() && PortalData.isPresses()) {
                        PressesEntity presses = PortalData.getPresses();
                        if (presses != null && PortalData.isCode()) {
                            redirectAttributes.addAttribute("pressesRouteName", presses.getPressesDataItemRouteName(PortalData.getCode()));
                        }
                    }
                    if(!PortalData.isSso() && PortalData.isTodo()) {
                        TodoEntity todo = PortalData.getToto();
                        if (todo != null && todo.getTodoNumber() > 0) {
                            redirectAttributes.addAttribute("todoRouteName", todo.getRouteName());
                        }
                    }
                    application.setLoginUserAuth("has_auth");
                    rpHolder.setClientUserAuth("has_auth");
                } else {
                    redirectAttributes.addAttribute("userAuth", "no_auth");
                    application.setLoginUserAuth("no_auth");
                    rpHolder.setClientUserAuth("no_auth");
                }

                rpHolder.setClientProviderId(providerId);
                rpHolder.setClientUserId(userId.toString());
                rpHolder.setClientUserType("browse");
                clientService.saveRPHolder(rpHolder);
            }
        }

//        System.out.println("auth_callback:  "+rpHolder.getClientUserAuth());
